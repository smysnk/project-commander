...
func TestParseLogLevel(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "trace", input: "trace", want: master.LevelTrace, ok: true},
...
func TestResolveConsoleLogLevelPrefersFlagOverEnv(t *testing.T) {
	t.Setenv("PC_MASTER_CONSOLE_LOG_LEVEL", "error")
...
func TestResolveConsoleLogLevelInvalidFallsBack(t *testing.T) {
	t.Setenv("PC_MASTER_CONSOLE_LOG_LEVEL", "chatty")
	level, raw, ok := resolveConsoleLogLevel("")
	if ok {
...
func TestFormatLogLevelLabel(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "trace", level: master.LevelTrace, want: "TRACE"},
